<template>
  ...
  >
    <div class="mb-large md:sticky content md:w-r11/24">
      <h2 v-if="title" v-kata-html="title" class="fade-up" />
      <p v-if="text" v-kata-html="text" class="mt-medium fade-up" />
      <KataLinks :links="links" class="mt-medium fade-up" />
    </div>
    <div
      v-if="list && list != null && list.length"
      class="mb-large grid grid-cols-1 lg:grid-cols-2 gap-small md:w-r12/24"
    >
