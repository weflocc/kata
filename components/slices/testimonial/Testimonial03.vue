<template>
  <div class="slice testimonial-3 w-r24/24 mx-auto">
    <h2
      v-if="title"
      v-kata-html="title"
    ...
    >
      <div
        v-for="item in list"
        :key="item._key"
        class="w-full md:w-1/2 md:px-6 md:py-3 pb-large fade-up"
      >
        <p v-if="item.quote" v-kata-html="item.quote" />
        <p v-if="item.name" v-kata-html="item.name" class="label-1 mb-0" />
        <p v-if="item.job" v-kata-html="item.job" class="para-4 mt-2" />
      </div>
    </div>
  </div>
</template>
